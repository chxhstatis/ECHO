...
from configuration import save_model_dir, test_image_dir, THRESHOLD
                    ...
                    if avgBkg <= (self._Bkg / 100.0):
                        ...
                        if PercentMasked >= (self._ROIpc / 100.0):
                            ...
                            if bool(SaveMasks) == True:
                                ...
                                resize(TileMaskO, (arr.shape[0], arr.shape[1], 3))
...
if __name__ == '__main__':
    ...
    try:
        rootpath = os.getcwd() + '/p1'
        slidepath = rootpath + '/' + os.listdir(rootpath)[0] + '/*.svs'
        print(slidepath)
    except IndexError:
...
def write_txt(prob, classname, path=os.getcwd(), name='index.txt'):
...
def get_size(image_dir):
    ...
    for root, dirs, files in os.walk(image_dir):
        for file in files:
            if os.path.splitext(os.path.join(root, file))[-1] in compat:
...
def get_coordDict():
    for root, dirs, files in os.walk(image_dir):
        for file in files:
            ...
            if os.path.splitext(file_full)[-1] in compat:
...
# 组合图片
def compose(composed_size):
    print(composed_size)
    to_image = Image.new('RGB', (composed_size[0] * image_size, composed_size[1] * image_size))
    for y in range(composed_size[1]):
        for x in range(composed_size[0]):
            if (x, y) in image_numDict:
                # print('{} is {}'.format((x, y), image_numDict[(x, y)]))
                image = Image.open(image_numDict[(x, y)])
                if os.path.split(image_numDict[(x, y)])[-1] in list0:
                    image = add_transparency(image)
                to_image.paste(image, ((x - 1) * image_size, (y - 1) * image_size))
                ...
                to_image.paste(image, ((x - 1) * image_size, (y - 1) * image_size))
    # to_image.show()
    to_image.save('test.jpeg', 'jpeg')


def add_transparency(img, factor=0.5, color=(0, 0, 0, 0)):
    img = img.convert('RGBA')
    layer = Image.new('RGBA', img.size, color)
    img = Image.blend(layer, img, factor)

    return img


def something():
    composed_size = get_size(image_dir)
    get_coordDict()
    compose(composed_size)
    ...
    for root, dirs, files in os.walk(test_image_dir):
        for f in files:
            ...
            if os.path.splitext(full_f1)[-1] == '.jpeg':
                ...
                # write_txt(probability, id_cls[idx])

                if not id_cls[idx] in dict:
                    dict[id_cls[idx]] = 0
                else:
                    dict[id_cls[idx]] = dict[id_cls[idx]] + 1

                if id_cls[idx] == id_cls[0]:
                    list0.append(f)
